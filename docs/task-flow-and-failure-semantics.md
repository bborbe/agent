...
On success, the output section holds the agent's extracted payload — the `output` value of its result envelope, i.e. the payload's content with any leading markdown section heading stripped — not the envelope itself, so the next phase can parse the section directly. The payload legitimately carries its own heading (typically a heading plus a fenced JSON block), and `Marshal` emits a section as `Heading` followed by `Body`, so writing the payload unstripped would emit the heading twice, bound the section at the second one to an empty body, and make the next phase fail with `json block missing`. `claude/agentStep.Run` strips a leading level-1/level-2 heading (the same boundary rule `IsMarkdownSectionHeading` applies; `### ` and deeper are kept) together with any immediately following blank line, leaving the fenced JSON as the body. Only the extracted-payload path is affected — the non-envelope fallback still writes its raw runner text.

This also changes what the `ShouldRun` failure check sees. With the duplicate heading gone, the section no longer bounds to an empty body, so `failureMarked`'s `parseAgentResultBody` now parses a non-empty body on re-dispatch. The outcome is unchanged: the payload's JSON is the phase's own schema (e.g. a plan) and carries no `status` field of the `AgentStatus` enum, so it is still not a failure marker and the step still skips — but the parse path is now live rather than vacuous.

`claude/agentStep.Run` therefore never writes a success-looking output section for a `needs_input`/`failed` runner body: it returns that status and the deliverer writes the `## Failure` marker as today (`delivery/content-generator.go`). Failure-marker detection is in-memory markdown inspection only (the `Step.ShouldRun` "guards must be cheap" contract).
