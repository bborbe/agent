...
<summary>
- Add TaskFrontmatter (typed map) and TaskFile types to lib/
...
<requirements>
### 1. Add `TaskFile` to `lib/`

Create `lib/agent_task-frontmatter.go`:
...
- **Empty frontmatter** — `req.Frontmatter` is empty `TaskFrontmatter`; verifies file is still written with empty frontmatter block
- **Frontmatter with nested values** — `req.Frontmatter` is a `TaskFrontmatter` containing lists and nested maps (e.g. `tags: [agent-task]`); verifies YAML serialization is correct
...
- `TaskFile` and `TaskFrontmatter` go in `lib/` (not in task/controller) — agents need these types to publish requests
