...
- Existing test at line 185 that asserts `phase: human_review` at cap is updated to assert `phase` unchanged and `assignee: ""`
...
- `task/controller/pkg/command/task_increment_frontmatter_executor.go` — specifically the `buildIncrementModifyFn` function at line 92-117, especially the cap-escalation block at lines 112-114
...
**What to change:** The cap escalation block must set `fm["assignee"] = ""` instead of `fm["phase"] = "human_review"`. The phase field must be left untouched.
</context>
...
2. **Update the existing test `Context("phase escalation at cap")` in `task/controller/pkg/command/task_increment_frontmatter_executor_test.go`**:
   - The existing test at line 158 asserts `Expect(fm["phase"]).To(Equal("human_review"))` at line 185
   - Change this assertion to `Expect(fm["phase"]).NotTo(Equal("human_review"))`
   - Add assertion `Expect(fm["assignee"]).To(BeEmpty())` to verify assignee is cleared
   - Rename the Context description from `"phase escalation at cap"` to `"trigger_count cap escalation"` to match spec terminology

   **Old test assertions (lines 183-185):**
   ```go
...
3. **Add new test cases for pre-write phase preservation** in the same `Context("trigger_count cap escalation")` block, after the existing escalation test:
...
4. **Run `make test`** in `task/controller/` to verify the tests pass:
   ```bash
...
<constraints>
- Only modify the cap-escalation block in `buildIncrementModifyFn` — no other changes to the function
...
<verification>
```bash
# AC1: No human_review write in cap path
grep -n 'phase.*=.*"human_review"' task/controller/pkg/command/task_increment_frontmatter_executor.go
# Expected: 0 matches in buildIncrementModifyFn (comment references are ok)
...
# AC4: Assignee empty assertion added
