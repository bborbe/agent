...
- [ ] Unit test: given a task file with `assignee: pr-reviewer-agent` and `phase: planning`, applying `UpdateFrontmatterCommand{Updates: {"phase": "human_review"}, Body: <Verdict section>}` produces frontmatter shape per AC#4 above PLUS body containing the `## Verdict` section. Evidence: test asserts both the frontmatter shape (assignee cleared, previous_assignee set) and the body-section presence in a single write.
...
- [ ] Live verification on dev cluster: deliberately reproduce the verify-fail scenario (force pr-reviewer's verify step to flag hallucinations, or use an equivalent reproducer that causes an agent to emit `UpdateFrontmatterCommand` with `phase: human_review`). Resulting task file has `assignee: ""`, `previous_assignee: pr-reviewer-agent` (or whichever agent emitted), `phase: human_review`, and the `## Verdict` (or equivalent agent body section) appended to the body. Operator-board inbox filter (`assignee == ""`) surfaces the task within one refresh. Evidence: captured task file path + relevant frontmatter snippet + screenshot or transcript of the inbox surface.
