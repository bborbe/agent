...
- Agent types become Kubernetes custom resources (`Config`) instead of a hardcoded slice in executor code
- Adding a new agent is a single `kubectl apply`; no Go change, no rebuild, no redeploy
- The executor watches Config CRs and keeps an in-memory lookup keyed by assignee
...
After this work the executor discovers agent types dynamically by watching `Config` custom resources in its namespace. Adding, removing, or updating an agent is a single `kubectl apply` (or `kubectl delete`) operation — the executor picks up the change without restart. The executor binary contains no knowledge of specific agents. Each Config CR declares image, per-agent env, secret reference, PVC reference, and mount path; the executor merges those with shared env vars (`TASK_CONTENT`, `TASK_ID`, `KAFKA_BROKERS`, `BRANCH`) and spawns the Job.

## Non-goals

- Changing the Kafka event schema or `lib.Task` structure
- Building a reconciling controller that updates Config status — the executor is a read-only consumer
...
1. `Config` (`apiVersion: agent.benjamin-borbe.de/v1`, namespace-scoped) is registered with an OpenAPI v3 schema covering every field in `docs/agent-crd-specification.md`: `assignee`, `image`, `heartbeat`, `resources`, `env`, `secretName`, `volumeClaim`, `volumeMountPath`
...
6. Deleting an Config CR causes subsequent tasks for that assignee to skip with a warning
7. Updating an Config CR (image, env, secret) is reflected on the next matching task, no restart

## Constraints

**Business / contract**:
- The existing `AgentConfiguration` struct keeps its current shape (image, env, secretName, volumeClaim, volumeMountPath) — it is the conversion target from `agentv1.Config`
...
- Authoritative Config CRD schema is `docs/agent-crd-specification.md`
...
- Config is namespace-scoped (matches executor scope, simplifies RBAC)
...
- The `agent.benjamin-borbe.de` API group is not already claimed in the cluster
...
|---------|-------------------|----------|
| No matching Config for assignee | Warn, increment `skipped_unknown_assignee`, skip message | Apply missing CR |
| Config CRD missing at startup | Executor creates it | Automatic |
| Config CRD exists with older schema | Executor updates it | Automatic |
...
| Config CR deleted while executor running | Next matching task skips with warning | Re-apply CR |
| Config CR updated | Next matching task uses new values | No action needed |
...
- Config CRs are applied by cluster admins only — RBAC on `configs.agent.benjamin-borbe.de` restricts who can create/update them
- The executor's service account has only read verbs (`get/list/watch`) on Config — a compromised executor cannot mutate its own agent catalog
...
- A malicious Config CR could point to a public registry image — mitigated by cluster-level image policy (out of scope) and by RBAC restricting who can apply CRs
...
- [ ] Config CRD self-installs on executor startup — `kubectlquant -n dev get crd configs.agent.benjamin-borbe.de` returns the CRD without any manual apply
- [ ] Config CRs applied in dev matching today's hardcoded configuration (including secretName, volumeClaim, volumeMountPath on trade-analysis)
...
- [ ] `kubectlquant -n dev delete config agent-trade-analysis` → next task for `trade-analysis-agent` skips with warning
...
- [ ] Executor ServiceAccount has the minimum RBAC required to self-install the CRD and watch Config resources
...
1. Deploy executor; confirm CRD self-installed:
   ```
   kubectlquant -n dev get crd configs.agent.benjamin-borbe.de
   ```
2. Apply Config CRs for each agent:
   ```
   kubectlquant -n dev apply -f task/executor/k8s/agent-claude.yaml
   kubectlquant -n dev get configs
   ```
