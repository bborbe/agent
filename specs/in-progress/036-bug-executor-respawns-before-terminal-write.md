...
- [ ] **`JobStartedAt() (time.Time, error)` accessor exists on the task frontmatter type** — evidence: `grep -rn 'func .* JobStartedAt' task/ lib/ 2>/dev/null` returns ≥1 line declaring a method that reads the existing `job_started_at` string field, parses it via the standard time format used by `result_publisher.go:75` (`2006-01-02T15:04:05Z07:00`), and returns `(time.Time, error)`. The accessor lives in whichever package currently owns the frontmatter type used by `task_event_handler.go` (verify with `grep -n 'CurrentJob()' task/ lib/` and add `JobStartedAt()` to the same type). A unit test covers parse-success, parse-failure (returns error), and missing-field (returns zero time + nil error to preserve the legacy-task semantics in DB #7).
...
- [ ] No per-agent override is added — evidence: `grep -rn 'RespawnGracePeriod' task/executor/pkg/handler/ task/executor/pkg/metrics/ task/controller/` returns exactly one hit (the constant definition); no `AgentConfiguration` field, no env var, no flag.
...
- [ ] `docs/task-flow-and-failure-semantics.md` documents the new "Executor respawn gates" contract — evidence: `grep -n 'respawn_grace_window\|Executor respawn gates' docs/task-flow-and-failure-semantics.md` returns ≥1 line under a new subsection that describes (a) clean-exit signal via `current_job` cleared by the agent on terminal write, (b) grace-period fallback for crash-retry, (c) the suppression log line + metric label. This keeps the gate's intent durable beyond the spec.
