...
- Migrate the create-task command + sender (introduced by spec 019 in the flat layout) into the new sub-package; retire the flat `agent/lib/agent_task-commands.go`.
- Operation strings on the wire are unchanged: `create-task`, `update-frontmatter`, `increment-frontmatter`.

## Problem

After spec 019 lands, `agent/lib` has a `Validate(ctx)` method and a sender helper for `CreateCommand` only — sitting alongside the flat `agent_task-commands.go` file that still hosts `UpdateFrontmatterCommand` and `IncrementFrontmatterCommand` without those affordances. Each remaining producer (e.g. the executor publishing frontmatter updates back to the controller) hand-rolls its marshal-and-send code with no shared validation barrier. The trading lib already established the per-command sub-package pattern; replicating it here makes the `Validate`-before-publish barrier the only sanctioned path for every command and removes the inconsistent half-flat layout left by spec 019.
...
2. The flat `agent/lib/agent_task-commands.go` file no longer exists; all three command structs and operation constants live in their per-command files.
...
7. Internal callers in the executor and controller (currently constructing flat-package commands and publishing via hand-rolled paths) are migrated to the new sub-package types and senders.
8. Operation constant strings on the wire are unchanged: `create-task`, `update-frontmatter`, `increment-frontmatter`.

## Constraints

- **Sequencing**: spec 019 (`human-readable-vault-task-paths`) must ship first. It introduces `CreateCommand.Validate` and the create-task sender helper in the flat layout; this spec relocates them into the new sub-package and replicates the pattern across the other two commands.
...
- [ ] `UpdateFrontmatterCommand.Validate` enforces required fields (`TaskIdentifier` non-empty, `Updates` non-empty when `Body` absent and vice versa per the existing schema).
- [ ] `IncrementFrontmatterCommand.Validate` enforces required fields (`TaskIdentifier` non-empty, `Field` non-empty).
...
- [ ] All internal callers in `agent/task/executor/` and `agent/task/controller/` that previously constructed `lib.{CreateTask,UpdateFrontmatter,IncrementFrontmatter}Command` are migrated to the new sub-package types.
- [ ] The previous flat-package sender helper for create-task (introduced by spec 019) is moved or retired in favor of the new sub-package sender.
