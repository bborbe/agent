...
- [ ] `grep -rn 'default:"todo"' --include='*.go' --exclude-dir=vendor` returns 0 lines for `TaskStatus`-typed flags (informational lines in usage strings allowed).
...
- [ ] At least one test file under `task/controller/pkg/scanner/` and one under `task/controller/pkg/command/` contains an assertion that round-trips `phase: in_progress` through `NormalizeTaskPhase` and expects the canonical `execution` back — `grep -n 'NormalizeTaskPhase' task/controller/pkg/scanner/*_test.go task/controller/pkg/command/*_test.go` returns ≥1 line.

## Verification

```bash
# In each touched module
