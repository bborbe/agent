...
- The executor watches Config resources and resolves assignee to configuration at runtime
...
After this work, the executor discovers agent types dynamically by watching `Config` custom resources in its namespace. Adding a new agent means `kubectl apply -f agent-config-foo.yaml` — no executor code change, no rebuild, no redeployment. Each Config declares its image, per-agent env vars (including secretKeyRef), resource limits, and heartbeat interval. The executor resolves assignee → Config at runtime via a shared informer.

## Non-goals

- Changing the Kafka event schema or task frontmatter format
- Building a controller/operator that reconciles Config (executor is the sole consumer)
...
1. A CRD `Config` is registered at `agent.benjamin-borbe.de/v1` with fields: assignee, image, env, resources, heartbeat
2. The executor starts a shared informer for Config in its namespace and maintains a local cache
...
8. Three Config manifests are created for existing agents: backtest-agent, trade-analysis-agent, claude
...
**Prerequisite:**
- Prompt `executor-agent-configuration` (Configuration struct) should be completed first — the CRD spec fields mirror `Configuration`
...
- The CRD API group `agent.benjamin-borbe.de` does not conflict with any existing CRDs in the cluster
...
|---------|-------------------|----------|
| No Config CRD for assignee | Log warning, skip event, increment `skipped_unknown_assignee` metric | Apply the missing Config manifest |
...
- CRD manifests are applied by cluster admins only (RBAC on `agent.benjamin-borbe.de` resources)
- Per-agent secrets stay in the agent's own K8s Secret, not in the executor's environment
- The executor only reads Config (get/list/watch) — it cannot create/modify CRDs
...
- [ ] CRD manifest registered: `kubectl get crd configs.agent.benjamin-borbe.de`
- [ ] Three Config resources applied (backtest-agent, trade-analysis-agent, claude)
