...
- fix: grant the executor's ServiceAccount `create`/`get`/`update`/`delete` on `apps/statefulsets`, so the service-agent reconcile loop can actually create the StatefulSet it exists to create. `executor-rbac.yaml` granted `batch/jobs` and `""/pods` and nothing under `apps` — because until the `type: service` work the executor never created a StatefulSet — so the loop ran correctly against a live cluster (`event=service_reconcile done configs=17 services=1 failures=1`) and then failed on the one Config that mattered: `create statefulSet failed: statefulsets.apps is forbidden: User "system:serviceaccount:dev:agent-task-executor" cannot create resource "statefulsets" in API group "apps"`. No SC, DoD row or task line named RBAC, so nothing in the task could have caught it before the first live reconcile. `delete` is granted from executor **v0.18.1** onwards only: that is the release in which `ServiceReconciler` checks the object's Config controller ownerRef before removing it, and pairing the rule with an older executor would arm an unchecked delete-by-name across every non-service Config in the namespace.

## v0.90.1

- fix: bump the chart version so the `spec.type` render added in v0.90.0 can actually reach the registry, and stop rendering an unused PVC for a `type: service` agent. Chart publishing is a manual `helm package` + `helm push` with no CI, and `helm/Chart.yaml` was left at `0.6.4` when the templates changed — so `oci://registry-1.docker.io/bborbe/agent:0.6.4` still predates the `type` render entirely (verified by pulling it: the only `type:` in its `templates/agents.yaml` is `type: Opaque` on a Secret, and its `crds/config-crd.yaml` declares no `type` property). A deploy pinned to `CHART_VERSION := 0.6.4` therefore renders a Config with **no `spec.type`**, the executor resolves it as a `job` agent, no StatefulSet is created, and nothing errors — the same silent-prune class as the `maxConcurrentJobs` v0.5.0 incident, arriving by a different route. Separately, `templates/agents.yaml` rendered `spec.volumeClaim` and a standalone `PersistentVolumeClaim` whenever `volumeMountPath` was set; a service agent's volume comes from the StatefulSet's own `volumeClaimTemplates` and the reconciler ignores `spec.volumeClaim`, so the chart was creating a second, unmounted claim that nothing could report as wrong. `volumeMountPath` still renders for a service agent, because the reconciler does honour it as the mount point.
...
- feat: make the Pi runner's session flag opt-out via `PiRunnerConfig.PersistSession`, so a long-running identity agent can keep continuity across prompts without changing what a task-routed agent does — `buildCommand` passed `--no-session` unconditionally, and that flag is load-bearing for job agents: `pi-agent` mounts a shared `volumeClaim` at `/home/pi/.pi`, so persisting sessions there would let a later run of a task-routed agent resume an unrelated earlier task's conversation. Default stays false, so every existing Pi agent is byte-identical in behaviour; only an identity agent sets it. `--no-session` remains the first flag after `--print --mode json` for a job agent, and the two new specs pin both directions.
...
- chore: update github.com/bborbe/kafka to v1.25.16, github.com/bborbe/metrics to v0.6.3, github.com/bborbe/vault-cli to v0.126.3

## v0.87.4

- chore: update github.com/bborbe/errors to v1.6.1, github.com/bborbe/kafka to v1.25.15, github.com/bborbe/metrics to v0.6.2, github.com/bborbe/time to v1.27.14, github.com/bborbe/validation to v1.5.0, github.com/bborbe/vault-cli to v0.126.1
...
- feat(helm): render `maxConcurrentJobs` on the agent Config CRD when `agents[].maxConcurrentJobs` is set; chart 0.5.0→0.5.1. Emitted only when present, so no existing agent's rendered Config changes. Consumed by `agent-task-executor` ≥ v0.4.8 ([#13](https://github.com/bborbe/agent-task-executor/pull/13)) to cap that agent's concurrent Jobs; older executors ignore the field.
  ...
  Needed because 20 `github-update-go` Jobs spawned within 40s on 2026-08-10 all exceeded their 1800s `activeDeadlineSeconds` through mutual contention. Measured uncontended, one such Job takes 669s (`cron`) to 903s (`kafka`), so the deadline is adequate and the concurrency is not.
